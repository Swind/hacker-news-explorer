...
updated_at: 2026-02-03T04:45:53
---
...
這是一起**嚴重的軟體供應鏈攻擊事件**。Notepad++ 官方網站的基礎設施在 2025 年 6 月至 12 月期間遭到入侵，攻擊者透過共享主機供應商的漏洞劫持了更新流量。多名獨立安全研究人員評估認為，攻擊者很可能是**中國國家贊助的 APT 組織**，這次攻擊展現了高度選擇性的定目標模式。

## 為什麼有趣

這是一個**里程碑式的安全事件**，具有多重意義：

1. **供應鏈攻擊的典型範例**：攻擊者不是入侵 Notepad++ 程式碼本身，而是透過主機供應商的基礎設施漏洞進行攻擊。這暴露了整個軟體生態系統的脆弱性。

2. **國家級 APT 的能力展示**：攻擊展現了高度精確的情報收集能力——他們專門搜索 `notepad-plus-plus.org` 網域，並針對特定用戶進行選擇性攻擊，而非大規模撒網。

3. **自動更新的信任危機**：用戶普遍信任官方更新機制，但這次事件表明，即使是官方來源也可能被劫持。這迫使整個行業重新思考「信任但驗證」的原則。

4. **政治與軟體安全的交織**：Notepad++ 作者 Don Ho 長期在軟體中加入政治訊息（如「Free Hong Kong」），這次被指為中國國家黑客攻擊的目標，引發了關於「軟體是否應該政治中立」的深度討論。

5. **小團隊開源專案的弱點**：Notepad++ 是一個小團隊維護的專案，缺乏資源投入完整的 DevSecOps 流程。這反映了整個開源生態系統的系統性風險。
...
**完整時間線：**
```
2025年6月      → 攻擊開始，共享主機伺服器遭入侵
2025年9月2日   → 主機廠商維護更新核心，攻擊者失去伺服器存取權
2025年9月-12月 → 攻擊者利用先前竊取的憑證持續劫持更新流量
2025年12月2日  → 憑證完全輪換，所有攻擊者存取權被終止
2026年2月2日   → 事件正式公開披露
```

**攻擊手法：**
- 攻擊載體：透過共享主機供應商的基礎設施級別漏洞
- 攻擊目標：`notepad-plus-plus.org/getDownloadUrl.php` 端點
- 利用漏洞：舊版本 Notepad++ 的更新驗證機制不足
- 選擇性攻擊：僅針對特定目標用戶，非全面性劫持
...
**v8.8.9 版本（已發布）：**
- WinGup 更新器增強：同時驗證憑證與數位簽章
- XML 更新清單加入 XMLDSig 簽章
...
**攻擊歸因的爭議：**
> 「他們怎麼知道是中國組織，甚至是國家贊助的？」

APT 歸因本質上是推測性的。在缺乏明確證據的情況下，歸因聲明應謹慎對待。

**政治與軟體的界線：**
- Notepad++ 作者的政治表達是否使工具成為報復目標？
- 開源軟體維護者是否有權在軟體中表達政治立場？
- 「保持政治中立」本身就是一種政治立場的觀點爭論

**供應鏈安全的防範建議：**
- 使用硬編碼公鑰（存放在 HSM 中最佳）而非依賴伺服器
- 採用 reproducible builds 確保編譯結果可重現驗證
- 實施多因素簽章驗證（如多個開發者簽名）
- 定期進行供應鏈安全審計
...
1. **技術價值**：這是教科書式的軟體供應鏈攻擊案例，展示了高級 APT 組織如何繞過傳統防禦，深入理解後可以幫助開發者建立更安全的更新機制。

2. **教育意義**：這次事件提醒整個開源社群，即使是廣受信任的工具也可能成為高級威脅行為者的目標。小團隊專案需要審視自身的安全流程。

3. **社會影響**：觸發了關於軟體中立性、政治表達權利與安全風險之間平衡的深度討論。

4. **行動建議**：
   - 用戶：更新至 v8.8.9 或等待 v8.9.2 強制簽章驗證版本
   - 開發者：重新評估專案的更新機制安全性，考慮實施客戶端簽章驗證
   - 組織：建立軟體物料清單（SBOM）並監控供應鏈風險

---

**備註**：由於攻擊者未留下具體的 IoC（指標），無法提供惡意檔案的雜湊值或 IP 位址。用戶應透過驗證官方版本的數位簽章來確認檔案完整性。